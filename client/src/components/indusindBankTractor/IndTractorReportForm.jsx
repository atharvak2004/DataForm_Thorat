...
export default function IndTractorReportForm({ onSubmit }) {
    ...
    const searchByRegistrationNo = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/report2/find/${searchInput}`);
    ...
    return (
        <div className=" p-10 text-xl ">
            <h1 className="text-4xl mb-14 font-semibold text-center">INDUSIND BANK LTD. TRACTOR REPORT</h1>
            <div className="bg-gray-200 rounded-xl p-10 max-w-5xl w-full mx-auto border border-gray-300 mb-10">
                <h2 className="text-2xl mb-2">Search by Vehicle Number</h2>
                <input type="text" placeholder="Enter Vehicle No (e.g. MH12AB1234)" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="p-2 border rounded w-1/2 mr-2" />
                <button type="button" onClick={searchByRegistrationNo} className="bg-green-600 text-white px-4 py-2 rounded" >Search
</button>
            </div>
