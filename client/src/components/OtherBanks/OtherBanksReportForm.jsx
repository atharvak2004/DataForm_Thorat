...
export default function OtherBanksReportForm({ onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editMode
        ? `${import.meta.env.VITE_API_URL}/report5/update/${rowIndex}`
        : `${import.meta.env.VITE_API_URL}/report5`;
  ...
  const searchByvehicleNo = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/report5/find/${searchInput}`);
